    ...
    class SimpleRouter implements Router
    {
        ...
        private function detectBasePath(string $basePath): string
        {
            ...
            if (isset($_SERVER['REQUEST_URI'])) {
                ...
                if (strpos($fullPath, '/' . basename(__FILE__)) === 0) {
...
namespace Tqdev\PhpCrudApi\Middleware {
    ...
    class ApiKeyDbAuthMiddleware extends Middleware
    {
        ...
        public function __construct(Router $router, Responder $responder, array $properties, ReflectionService $reflection, GenericDB $db)
        {
            ...
            $router->register('POST', '/login', array($this, 'handleRequest'));
            $router->register('POST', '/register', array($this, 'handleRequest'));
            $router->register('POST', '/password', array($this, 'handleRequest'));
        }
...
namespace Tqdev\PhpCrudApi\Middleware {
    ...
    class DbAuthMiddleware extends Middleware
    {
        ...
        public function __construct(Router $router, Responder $responder, array $properties, ReflectionService $reflection, GenericDB $db)
        {
            ...
            $router->register('POST', '/register', array($this, 'handleRequest'));
            $router->register('POST', '/password', array($this, 'handleRequest'));
            $router->register('POST', '/logout', array($this, 'handleRequest'));
            $router->register('GET', '/me', array($this, 'handleRequest'));
        }
